...
def getWordList(file_path):
    ...
    word_series = word_csv.loc[:TESTNUM, "Word"]
    print(f"Debug: the current word_series is: \b {word_series}")
    print(f"Debug: the word_series contains {word_series.dtype} type data.")
    return word_series
...
def main():
    ...
    for wd in word_series:
        ...
        current_word_dict = parseInfo(getJSON(wd))
